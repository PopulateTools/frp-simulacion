...
document.addEventListener('DOMContentLoaded', function () {
  for (var _i = 0, _tooltipInfo = tooltipInfo; _i < _tooltipInfo.length; _i++) {
    var args = _tooltipInfo[_i];
...
var barChart = function barChart(id, csv, legend, tableClass, scaleMinY, scaleMaxY) {
  ...
  // legends
  svg.append('text').attr('class', 'legend-top').attr("x", 10).attr("y", 20).text(legend);
  ...
  var updateChart = function updateChart(dataz) {
    ...
    chart.selectAll('.container-grouped').data(dataDifNetAcu).on('mouseover', function (d) {
      ...
      tooltipSimulationBarChart.style('opacity', 1).html("\n                  <span class=\"tooltip-bar-chart-year\"><strong>A\xF1o:</strong> ".concat(d.year, "</span>\n                  <span class=\"tooltip-bar-chart-dif-neta\"><strong class=\"rect-before\">Diferencia neta:</strong> ").concat(d.neta.toLocaleString(), " ").concat(legend, "</span>\n                  <span class=\"tooltip-bar-chart-dif-acumulada\"><strong class=\"rect-before-fluor\">Acumulada:</strong> ").concat(d.acumulada.toLocaleString(), " ").concat(legend, "</span>\n            ")).style('left', "".concat(margin.left * 2, "px")).style('top', "".concat(margin.top, "px"));
    ...
    d3.csv("csv/".concat(csv, ".csv"), type, function (error, data) {
      if (!error) {
        ...
        simulation.selectAll("span").data(function (d) {
          return ["simulacion"].map(function (key) {
            return {
              key: key,
              value: d[key]
            };
        ...
        simulation.selectAll(".simulation-percentage").data(function (d) {
          return ["simulacionpercentage"].map(function (key) {
            return {
              key: key,
  ...
  function getArrayDifNetaAcumula(data) {
    ...
    for (var i = 0; i < data.length; i++) {
      var difNeta = data[i].simulacion - data[i].prevision;
    ...
    arrayDifNetaAcumula = arrayDifNeta.map(function (_, index) {
...
var multipleLine = function multipleLine() {
  ...
  if (multipLeFired === false) {
    svg.append('text').attr('class', 'legend-top').attr("x", 0).attr("y", 20).text('Miles de M €');
  }
  var setupScales = function setupScales() {
  ...
  var updateChart = function updateChart(data) {
    ...
    var line = d3.line().x(function (d) {
      ...
      return scales.count.y(d.simulacion);
    });
    ...
    d3.selectAll('.line').data(dataComb).on('mouseover', function (d) {
      ...
      for (var i = 0; i < d.values.length; i++) {
        var _element = d.values[i].simulacion;
  ...
  function update(filter) {
    d3.csv('csv/simulation-all.csv', function (error, data) {
      if (error) {
        console.log(error);
      } else {
  ...
  d3.select("#empleo-view").on("click", function () {
    d3.csv('csv/simulation-all.csv', function (error, data) {
      if (error) {
        ...
        svg.append('text').attr('class', 'legend-top').attr("x", 0).attr("y", 20).text('Miles');
        updateChart(data);
      }
    });
  });
